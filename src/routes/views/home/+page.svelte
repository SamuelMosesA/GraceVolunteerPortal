<script lang="ts">
	...
	function openRoster(roster: Roster | null | undefined) {
		if (roster) {
...
<div class="max-w-lg justify-self-center md:min-w-lg">
	...
	{:else if schedule && schedule.volunteers && schedule.volunteers.length > 0}
		{#each schedule.volunteers as volunteer}
			{@const upcomingShifts = getUpcomingShifts(volunteer.shifts)}
			...
			<div class="mt-12">
						...
						<Card.Content class="p-0">
							<div class="flex flex-col items-center gap-6 md:flex-row md:items-stretch">
								...
								<div class="flex flex-1 flex-col justify-center text-center md:text-left">
									...
									{#if nearestShift.teamShiftDetails}
										<div class="mt-4 flex justify-center md:justify-start">
											<Button
												variant="outline"
												size="sm"
												class="rounded-full"
												onclick={() => openRoster(nearestShift?.teamShiftDetails)}
											>
				...
				<h3 class="text-primary mt-8 mb-4 text-xl font-semibold">Full Roster</h3>
								...
								<Table.Row>
									...
									<Table.Cell>
										<div class="flex items-center justify-between gap-4">
											...
											{#if shift.teamShiftDetails}
												<Button
													variant="ghost"
													size="sm"
													class="h-8 px-2 text-xs"
													onclick={() => openRoster(shift.teamShiftDetails)}
												>
		...
		{/each}
	{:else}
		<p class="text-muted-foreground mt-8">No schedule found for your account.</p>
	{/if}
</div>
